...
|------------|------------|
| Денежный ящик | только **наличные** (`CASH`), с учётом возвратов |
| Общий баланс кассы | **нал + безнал** (`CASH` + `CREDIT_CARD`) |
| Зачёт аванса | `PRE_PAID` — отдельно, в общий баланс не входит |

Суммы в API уже со знаком: «+» приход, «−» возврат. На экране — таблицы, графики (по типам оплаты, приход/возврат, балансы) и выгрузка в **Excel**. История последних отчётов хранится в сессии до перезапуска сервера.
