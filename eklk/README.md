...
| Раздел | Что делает |
|--------|------------|
| **Главная** | Дашборд: платежи/чеки, графики по типам оплаты |
| **Чек** | Фискальный чек (sell / sell_refund / buy / buy_refund), агенты, доп. реквизиты |
| **Ссылка на оплату** | Invoice через платёжки (Сбер, Тинькофф и т.д.) |
| **Многоразовые ссылки** | Шаблоны QR Pay, постоянные ссылки |
| **Список чеков** | Поиск транзакций (Mobile API), состав чека, ссылка ОФД |
| **Каталог** | Номенклатура: список, поиск, CRUD, массовое удаление, импорт CommerceML 2 |
| **Отчёты** | Сводки за период, баланс кассы (нал / нал+безнал / зачёт), графики, XLS |
| **ИИ кассир** | Встроенный чат-помощник (embed) |
| **Настройки** | Организация, магазины, тема (светлая / тёмная / glass); кнопка ⚙ у «Выйти» |
...
- Вкладки / URL: home, create, payment, templates, orders, catalog, reports, ai-cashier; settings — ⚙ (клиентский роутинг).
...
Отдельные вкладки ЛК (не часть монолита `app.js`). Данные только из API EcomKassa, локально не кэшируются (кроме краткой истории отчётов в сессии сервера).

### Каталог

- **Список и поиск** — Mobile API (`Token` как при входе в ЛК).
- **Создание / изменение / удаление** — сервис `catalog.ecomkassa.ru`, тело как у мобильного клиента EcomKassa:
  - новый товар: `"itemId": -1` (ID назначает EcomKassa);
  - `vatType` / `paymentObject` в нижнем регистре (`vat20`, `commodity`);
  - в теле передаётся `taxIdentity` (ИНН).
- **Артикул (SKU):** если не указан — генерируется; если уже занят — ошибка (EcomKassa не допускает дубликаты).
- **Импорт CommerceML 2** (XML): лишние поля пропускаются; модификации товара → отдельные позиции. После импорта показывается отчёт: сколько создано, ошибок, сгенерировано артикулов и т.д.
- **Удалить все** — нужно ввести слово `удалить` для подтверждения.

> Запись в каталог с некоторых сетей может отвечать 401: авторизация сервиса каталога ещё уточняется. Просмотр списка при этом работает.

### Отчёты

Удобные сводки для бухгалтера за день / неделю / месяц / квартал / год.

Суммы в API приходят в **копейках**, в ЛК показываются в **рублях**.  
**Баланс кассы за период** = сумма всех сумм из ответа EcomKassa (без отдельного пересчёта возвратов).  
На экране — разбивка по типам оплаты, графики и выгрузка в **Excel**. История последних отчётов хранится в сессии до перезапуска сервера.

Подробности для разработки и ИИ: [`AGENTS.md`](AGENTS.md) §12.
